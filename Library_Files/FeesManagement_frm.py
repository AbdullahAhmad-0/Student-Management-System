...
class FeesManagement(BeforeInIt, AllSettings, CommonFunction):
    wSize, hSize = 800, 550
    title = "Fees Management - Student Management System"
    mainName = "Fees Management"
    ...
    def __init__(self, wind, libFormList=[]) -> None:
        ...
        frm_fees_ = Frame(self.detailFrame, bg=self.colorList[2], width=self.dFbF_w - 10, height=y_space)
        frm_fees_.pack(fill=BOTH, expand=True)
        lbl_fees_ = Label(frm_fees_, text='Total Fees', font=self.formset_mainF, bg=self.colorList[7], fg=self.colorList[8], highlightbackground=self.colorList[11], highlightthickness=2, highlightcolor=self.colorList[12])
        lbl_fees_.place(x=10, y=1)
        ent_fees_ = Entry(frm_fees_, textvariable=self.var_fees_, border=0, font=self.formset_mainF, bg=self.colorList[9], fg=self.colorList[10], highlightbackground=self.colorList[11], highlightthickness=2, highlightcolor=self.colorList[12])
        ent_fees_.place(x=self.dF_ent_x, y=1, width=self.dF_ent_w, height=self.formset_fieldH)

        frm_submit_ = Frame(self.detailFrame, bg=self.colorList[2], width=self.dFbF_w - 10, height=y_space)
        frm_submit_.pack(fill=BOTH, expand=True)
        lbl_submit_ = Label(frm_submit_, text='Submitted Fees', font=self.formset_mainF, bg=self.colorList[7], fg=self.colorList[8], highlightbackground=self.colorList[11], highlightthickness=2, highlightcolor=self.colorList[12])
        lbl_submit_.place(x=10, y=1)
        ent_submit_ = Entry(frm_submit_, textvariable=self.var_submit_, border=0, font=self.formset_mainF, bg=self.colorList[9], fg=self.colorList[10], highlightbackground=self.colorList[11], highlightthickness=2, highlightcolor=self.colorList[12])
        ent_submit_.place(x=self.dF_ent_x, y=1, width=self.dF_ent_w, height=self.formset_fieldH)
        ...
        self.Table = ttk.Treeview(self.recordFrame, columns=['id', 'date_', 'sroll_', 'student_', 'technology_', 'class_', 'shift_', 'fees_', 'submit_'], yscrollcommand=scroll_y.set, xscrollcommand=scroll_x.set)
        ...
        self.Table.heading('fees_', text='Total Fees')
        self.Table.heading('submit_', text='Submitted Fees')
        ...
        self.Table.column('fees_', width=100)
        self.Table.column('submit_', width=100)
    ...
    def Variables(self):
        ...
        self.var_fees_ = StringVar()  # Total Fees
        self.var_submit_ = StringVar()  # Submitted Fees
    ...
    def Export_def(self):
        ...
        expList = list(zip(id, date_, sroll_, student_, technology_, class_, shift_, fees_, submit_))
    ...
    def Get_Data(self, e):
        try:
            ...
            self.var_sroll_.set(row[2])
            self.var_student_.set(row[3])
            self.var_technology_.set(row[4])
            self.var_class_.set(row[5])
            self.var_shift_.set(row[6])
            self.var_fees_.set(row[7])
            self.var_submit_.set(row[8])
    ...
    def StarterDb(self, db='sqlite'):
        if db == 'sqlite':
            ...
            self.cur.execute(
                ...
                , fees_ TEXT NOT NULL
                , submit_ TEXT NOT NULL )''')
            self.con.commit()

    def __list__(self, fetch):
        list_var = [self.var_id, self.var_date_, self.var_sroll_, self.var_student_, self.var_technology_, self.var_class_, self.var_shift_, self.var_fees_, self.var_submit_]
        list_shn = ['id', 'date_', 'sroll_', 'student_', 'technology_', 'class_', 'shift_', 'fees_', 'submit_']
        list_cvar = [[self.var_id, 'ID'], [self.var_date_, 'Date'], [self.var_sroll_, 'Student Roll Number'], [self.var_student_, 'Student Name'], [self.var_technology_, 'Technology'], [self.var_class_, 'Class'], [self.var_shift_, 'Shift'], [self.var_fees_, 'Total Fees'], [self.var_submit_, 'Submitted Fees']]
        list_head = ['ID', 'Date', 'Student Roll Number', 'Student Name', 'Technology', 'Class', 'Shift', 'Total Fees', 'Submitted Fees']
        list_headA = ['ID', 'Date', 'Student Roll Number', 'Student Name', 'Technology', 'Class', 'Shift', 'Total Fees', 'Submitted Fees']
